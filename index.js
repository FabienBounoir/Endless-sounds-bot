...
function play()
{
    ...
    channel.join().then(connection => {
        ...
        dispatcher = connection.play( "./musique/" + musiqueEnCours,{volume: 0.5,});
...
client.on('message', async message => {
    ...
    if(message.content.toLowerCase() == "!statpike")
    {
        ComptePlayEgirl.find().exec((erreur, playCount) => {
            if (erreur) return console.error(err);
            

            if(musiqueEnCours == "pike-e-girl.mp3") //62724 play de egirl de pike avant ajoue autre musique
            {
                const XpEmbed = new MessageEmbed()
                .setColor('#7100fc')
                .setTitle("Nombres de Musique joué par Pike'Bot")
    ...
    if(message.content.toLowerCase() == "!reloadpike")
    {
...
.setFooter("Pike'Bot",client.user.avatarURL())
.setTitle("Parole Pike Egirl")
.setDescription(`REFRAIN:
